...
let print_ty : ty -> PPrint.document =
  let rec print t =
    let print_self = print and print_next = print_atom in
    match t with
...
let print_term : term -> PPrint.document =
  ...
  and print_left_open t =
    let print_self = print_left_open and print_next = print_app in
    PPrint.group
    @@
    match t with
    | Abs (x, tau, t) ->
    ...
    | other -> print_next other
  and print_app t =
    let print_self = print_app and print_next = print_atom in
    PPrint.group
    @@
    match t with

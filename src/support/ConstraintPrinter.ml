module Make (T : Utils.Functor) = struct
  ...
  let print_sat_constraint (c : sat_constraint) : PPrint.document =
    let rec print_top c = print_left_open c
    and print_left_open =
      let _print_self = print_left_open and print_next = print_conj in
      fun ac ->
    ...
    and print_conj =
      let _print_self = print_conj and print_next = print_atom in
      function
